...
**When cloning:** edit `lib/brand.ts` → `contact.email`, `contact.phone`, `contact.phoneDisplay`, `social.facebook`. Footer, Prices group card, and InformationsPrices group card all import from `brand` — no per-file hunts needed. Still check `RegiondoWidget.tsx` fallback and JSON-LD schema in `app/layout.tsx` manually.

---

## 2b. Hardcoded prices (learned from Vannes — now in `lib/brand.ts`)

| File | What | Now in |
|---|---|---|
| `components/sections/Prices.tsx` | `individualPrices` + `earlyBirdPrices` arrays at top of file | `brand.prices.individual` + `brand.prices.earlyBird` |
| `components/sections/InformationsPrices.tsx` | Inline price strings `"8,50€"`, `"5€"`, `"7,00€"`, `"3,50€"` | `brand.prices.*` |

**When cloning:** edit `lib/brand.ts` → `prices` object. Both components import from brand and update automatically.

**Audit:** `grep -rn '"8,50€"\|"5€"\|"7,00€"\|"3,50€"' components/` — should return zero in a correctly-cloned site.
...
### ⚠️ SVG fill trap (learned from Vannes)

Several SVGs have Carnac purple **baked directly into their `fill` attributes** — not applied via CSS class. These are invisible to a Tailwind grep and won't be caught by the hex audit above unless you also check `public/figma-assets/`.

**Audit command (run at Phase 2, before any other work):**
```bash
grep -rn "#4d1c64\|#54206d" public/figma-assets/
```

**Recolor command if hits found:**
```bash
find public/figma-assets -name "*.svg" -exec sed -i '' 's/#4d1c64/#f7a427/g; s/#54206d/#f7a427/g' {} +
```

**SVGs confirmed affected in Vannes build:** `icon-train.svg`, `Icon01.svg`, `Icon02.svg`, `Icon03.svg`, `Icon04.svg`, `Icon05.svg`, `PurpleCashIcon.svg`, `CalendarIconBig.svg`, `WeatherIconBig.svg`.
...
### ⚠️ Image folder rule — MANDATORY (learned from Vannes)

Every client delivers images in a folder on the Desktop named `[city]-images` (e.g. `vannes-images`, `quiberon-images`). **Only use images from that folder.** Never grab from another site's `public/figma-assets/`, the Carnac repo, or any other source.

Before starting image work, confirm the folder exists:
```bash
ls ~/Desktop/[city]-images/
```

**Pipeline for every image:**
```bash
sips -Z 1920 --setProperty format jpeg --setProperty formatOptions 52 \
  ~/Desktop/[city]-images/SourceFile.jpg \
  --out public/figma-assets/target-name.jpg
```

Target: < 1MB per image. Use quality 40 for hero bgs, 52 for section images, 75 for small icons.

This rule exists because mixing images from different cities caused client-visible errors (wrong city photos on the live site).

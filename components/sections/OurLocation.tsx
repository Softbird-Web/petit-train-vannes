...
const itemConfig: Record<(typeof itemKeys)[number], { icon: string; mapsUrl: string | null }> = {
  item1: {
    ...
    mapsUrl: "https://www.google.com/maps/dir/?api=1&destination=Place+Gambetta+Vannes+France",
  },
  item2: { icon: "/figma-assets/icon-car.svg", mapsUrl: null },
  item3: { icon: "/figma-assets/icon-train-sm.svg", mapsUrl: null },
};
    ...
    <section data-anim-section className="bg-[#f5ebdd] py-16 xl:py-28 isolate">
      <div className="max-w-[1280px] mx-auto px-5 xl:px-0 w-full flex flex-col-reverse lg:flex-row items-start lg:items-center gap-12 lg:gap-20">
        ...
        <div data-anim-item className="flex-1 flex flex-col gap-6 max-w-[623px]">
          ...
          <h2 className="font-normal font-['Bricolage_Grotesque',sans-serif] text-[32px] sm:text-[40px] md:text-[48px] text-[#1c1b29] leading-[1.2] tracking-[-1.5px] sm:tracking-[-2.5px] md:tracking-[-3.36px] max-w-[472px] break-words">
            {t("heading")}
          </h2>
              ...
              return (
                <div key={key} className="flex items-start gap-4">
                  ...
                  <div className="flex flex-col gap-2 pt-2.5 flex-1">
                    <p className="font-['Bricolage_Grotesque',sans-serif] text-[24px] text-[#1c1b29] leading-[1.2] tracking-[-1.68px]">
                      {t(`${key}Title`)}
                    </p>
